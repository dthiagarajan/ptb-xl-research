"""Monitoring callbacks for looking at input and activation histograms. Copied heavily from [1].

References:
[1]: https://github.com/awaelchli/pytorch-lightning-snippets
"""
...
class DataMonitorBase(Callback):
    ...
    def log_histograms(self, batch, group="") -> None:
        ...
        """
        if not self._log or (self._train_batch_idx is None) or (self._train_batch_idx + 1) % self._log_every_n_steps != 0:
            return
    ...
    def log_histogram(self, tensor: Tensor, name: str) -> None:
        ...
        logger.add_histogram(name, tensor, self._trainer.global_step)
