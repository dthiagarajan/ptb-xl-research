"""Resnet model implementations for signal data."""
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_input_channels=3, num_classes=1000,
                 ...
                 replace_stride_with_dilation=None, norm_layer=nn.BatchNorm1d):
        ...
        self.conv1 = nn.Conv1d(
            num_input_channels, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False
        )
...
def load_state_dict_from_url(url, progress=False):
    raise NotImplementedError("Function isn't implemented as pretrained models don't exist yet.")
...
def wide_resnet18_2(pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('wide_resnet18_2', BasicBlock, [2, 2, 2, 2],
                   pretrained, progress, **kwargs)


def wide_resnet34_2(pretrained=False, progress=True, **kwargs):
    r"""Wide ResNet-34-2 model from
    `"Wide Residual Networks" <https://arxiv.org/pdf/1605.07146.pdf>`_
    The model is the same as ResNet except for the bottleneck number of channels
    which is twice larger in every block. The number of channels in outer 1x1
    convolutions is the same, e.g. last block in ResNet-50 has 2048-512-2048
    ...
    """
    kwargs['width_per_group'] = 64 * 2
    return _resnet('wide_resnet34_2', BasicBlock, [3, 4, 6, 3],
                   pretrained, progress, **kwargs)
